...
class Controller_Cliente:
    ...
    def inserir_cliente(self) -> Cliente:
        ...
        if self.verifica_existencia_cliente(oracle, cpf):
            ...
            oracle.write(f"insert into cliente values ('{cpf}', '{nome}')")
            # Recupera os dados do novo cliente criado transformando em um DataFrame
            df_cliente = oracle.sqlToDataFrame(f"select cpf, nome from cliente where cpf = '{cpf}'")
    ...
    def atualizar_cliente(self) -> Cliente:
        ...
        if not self.verifica_existencia_cliente(oracle, cpf):
            ...
            oracle.write(f"update cliente set nome = '{novo_nome}' where cpf = {cpf}")
            # Recupera os dados do novo cliente criado transformando em um DataFrame
            df_cliente = oracle.sqlToDataFrame(f"select cpf, nome from cliente where cpf = {cpf}")
            # Cria um novo objeto cliente
    ...
    def excluir_cliente(self):
        ...
        if not self.verifica_existencia_cliente(oracle, cpf):            
            ...
            oracle.write(f"delete from cliente where cpf = {cpf}")            
